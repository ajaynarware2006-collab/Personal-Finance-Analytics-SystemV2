...
def highest_expense(user_id):
    ...
    if all_expenses == None:
        return None
    cursor.execute("SELECT MAX(amount) FROM user_view;")
...
def lowest_expense(user_id):
    ...
    cursor.execute("SELECT MIN(amount) FROM user_view;")
...
def category_summary(user_id):
    cursor.execute("SELECT category_name,MIN(amount),MAX(amount) FROM user_view WHERE user_id=%s GROUP BY category_name;",(user_id,))
...
def budget_remaining(user_id):
    ...
    if budget < total_expesnse:
        return "Out of budget now."
    else:
